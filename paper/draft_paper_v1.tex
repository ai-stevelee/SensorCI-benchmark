...
% Submission (double-blind): no option   |   Camera-ready: [final]   |   Preprint: [preprint]
...
\item[] Answer: \textbf{[Yes]}
\item[] Justification: Released new assets are: (i) persona-level schema files, (ii) 750 role-attribute sharing labels ($\Phi_D$), (iii) 250 algebraic queries (5 datasets $\times$ 50/each), (iv) per-item LLM-judge audit logs, (v) 3{,}257-row evaluation jsonl, (vi) configs/hp\_table.yaml with the validation-selected hyperparameters per cell. A Croissant metadata file with RAI fields documents schema, license, intended use, limitations, and dual-use considerations. Inline documentation is provided in Sec.~\ref{sec:datasets} (schema), Sec.~\ref{sec:judge} (curation protocol), and Sec.~\ref{sec:defenses} (evaluation).
